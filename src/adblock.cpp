...
static void load_skipper_script() {
    // UPDATED: Look in ../assets/ first
    std::ifstream t("../assets/ad_skipper.js");
    if (!t.is_open()) t.open("ad_skipper.js"); // Fallback

    if (t.is_open()) {
        std::stringstream buffer;
        buffer << t.rdbuf();
        std::string s = buffer.str();
        ad_skipper_script = g_strdup(s.c_str());
        g_print("AdBlock: Loaded JS from assets.\n");
    } else {
        g_print("AdBlock: Warning - ad_skipper.js not found in ../assets/ or root.\n");
    }
}

static void inject_css(WebKitUserContentManager *manager) {
    const char *css =
        "ytd-banner-promo-renderer, ytd-display-ad-renderer, .adsbygoogle, "
        "div[id^='google_ads_'], .ytp-ad-module { display: none !important; }";
...
void setup_adblock() {
    ...
    // UPDATED: Look in ../assets/
    std::ifstream t("../assets/blocklist.json");
    if (!t.is_open()) t.open("blocklist.json");
    ...
    if (t.is_open()) {
        ...
        g_bytes_unref(bytes);
        g_print("AdBlock: Loaded Blocklist.\n");
    } else {
        g_print("AdBlock: Warning - blocklist.json not found.\n");
    }
    load_skipper_script();
}
...
void enable_adblock_for_webview(WebKitWebView *view) {
    ...
    webkit_user_content_filter_store_load(adblock_store, "MalakAdBlock", NULL,
        (GAsyncReadyCallback)+[](GObject *src, GAsyncResult *res, gpointer data){
            ...
            if(filter) {
                webkit_user_content_manager_add_filter(WEBKIT_USER_CONTENT_MANAGER(data), filter);
